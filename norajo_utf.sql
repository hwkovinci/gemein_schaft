...
title_year NUMBER(4),
title_released DATE,
title_runtime VARCHAR2(50),
title_plot VARCHAR2(800),
title_country VARCHAR2(70),
title_poster VARCHAR2(250))
ORGANIZATION EXTERNAL(
TYPE ORACLE_LOADER DEFAULT DIRECTORY NORAJO
...
LOCATION(NORAJO: 'insert_title.dat' )
)
PARALLEL
...
CREATE TABLE title_list(
    ...
    title_runtime     VARCHAR2(50)  NOT NULL,
    title_plot        VARCHAR2(1000) NOT NULL,
    title_country     VARCHAR2(70)  NOT NULL,
    title_poster      VARCHAR2(500) NOT NULL);
...
LOCATION(NORAJO: 'list_actor.dat' )
)
PARALLEL
REJECT LIMIT UNLIMITED;
...
LOCATION(NORAJO: 'list_director.dat' )
)
PARALLEL
REJECT LIMIT UNLIMITED;
...
LOCATION(NORAJO: 'list_genre.dat' )
)
PARALLEL
REJECT LIMIT UNLIMITED;
...
LOCATION(NORAJO: 'insert_actor.dat' )
)
PARALLEL
REJECT LIMIT UNLIMITED;
...
LOCATION(NORAJO: 'insert_director.dat' )
)
PARALLEL
REJECT LIMIT UNLIMITED;
...
LOCATION(NORAJO: 'insert_genre.dat' )
)
PARALLEL
REJECT LIMIT UNLIMITED;
...
where 
title_id not in (select title_id from title_list) order by title_id desc;
